...
dispatch_expr :  expr '.' OBJECTID '(' expr_list ')' 
                  { $$ = dispatch($1, $3, $5); }
...
case_expr : CASE expr OF case_branches ESAC
                { $$ = typcase($2, $4); }
          ;
...
let_expr  : OBJECTID ':' TYPEID op_assign IN expr %prec FLAG
                { $$ = let($1, $3, $4, $6); }
          | OBJECTID ':' TYPEID op_assign ',' let_expr 
                { $$ = let($1, $3, $4, $6); }
          | error IN expr %prec FLAG { yyerrok; }
